...
class TestMediaMathLineItem(TestCase):
  ...
  def test_connection(self):
    ...
    session_id = line_item.t1.session_id
    self.assertIsNotNone(session_id)

  def test_get_lineitem_by_id(self):
    line_item = lineitem.LineItem()
    li = line_item.get_by_id(1197492)
    li = json.loads(li)
  ...
  def test_create_lineitem(self):
    ...
    new_lineitem = line_item.create(data)
  ...
  def test_update_strategy_domain_restrictions(self):
    line_item = lineitem.LineItem()
    li = line_item.update_strategy_domain_restrictions(1197492, ['youtube.com', 'apple.com'])
    li = json.loads(li)
    self.assertIn(li['msg_type'], 'success')
